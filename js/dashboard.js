...
function renderReports() {
  ...
  if (!container) return;
  if (WG.state.scans.length === 0) {
    container.innerHTML = `<div class="empty-state"><i class="fas fa-file-shield"></i><p>No reports yet. Run a scan to generate your first report.</p><button class="btn-primary-cta" onclick="showPage('scan')"><i class="fas fa-radar"></i> Start Scan</button></div>`;
    return;
  }
  container.innerHTML = WG.state.scans.map(s => {
    const col = s.score >= 70 ? 'green' : s.score >= 50 ? 'orange' : 'red';
    return `
    <div class="report-card">
      <div class="report-icon ${col}"><i class="fas fa-file-shield"></i></div>
      <div class="report-info">
        <div class="report-title">${s.url}</div>
        <div class="report-meta">${s.depth.charAt(0).toUpperCase()+s.depth.slice(1)} Scan &middot; ${s.vulns.length} vulns &middot; Score: ${s.score}</div>
      ...
      <div class="report-actions">
        ...
        <button class="btn-download" onclick="exportPDF('${s.id}')"><i class="fas fa-download"></i> PDF</button>
      </div>
    </div>`;
  }).join('');
}
...
function openReport(scanId) {
  ...
  document.getElementById('exportPdfBtn').onclick = () => exportPDF(scanId);
...
function startScan() {
  ...
  const stepInterval = setInterval(() => {
    ...
    if (stepIdx < totalSteps) {
      ...
      // Finalise scan
      completedScan = WG.addScan(url, depth, appType);
